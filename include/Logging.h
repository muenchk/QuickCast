...
public:
	...
	static void InitializeLog(std::filesystem::path _path, std::string name, bool append = false, bool timestamp = false);
};
...
public:
	...
	static void Init(std::string pluginname, bool append = false)
	{
		...
		//}
		if (!append)
			_stream = new std::ofstream(Logging::log_directory / (pluginname + "_usage.log"), std::ios_base::out | std::ios_base::trunc);
		else
			_stream = new std::ofstream(Logging::log_directory / (pluginname + "_usage.log"), std::ios_base::out | std::ios_base::ate);
		if (_stream == nullptr || _stream->is_open() == false)
			std::cout << "Cannot create Log!\n";
		lock.release();
	}
