...
namespace ScriptObject
{
	ScriptObjectPtr FromForm(const RE::TESForm* a_form, const std::string& a_scriptName)
	{
		...
		if (!a_scriptName.empty()) {
			...
			if (!object) {
				loginfo("Script {} is not attached to form. {}"sv, a_scriptName, Utility::GetHex(a_form->GetFormID()));
			}
		} else {
			...
			if (auto it = vm->attachedScripts.find(handle); it != vm->attachedScripts.end()) {
				...
				if (size == 1) {
					...
					loginfo("No scripts attached to. {}"sv, Utility::GetHex(a_form->GetFormID()));
					return nullptr;
				} else {
					loginfo("Multiple scripts are attached to. {}"sv, Utility::GetHex(a_form->GetFormID()));
					return nullptr;
				}
			}
		}
