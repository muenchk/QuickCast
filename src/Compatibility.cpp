...
void Compatibility::Load()
{
	...
	// NPCsUsePotions
	if (const uint32_t index = Mods::GetPluginIndex(NPCsUsePotions); index != 0x1) {
		loginfo("Found plugin NPCsUsePotions.esp.");
		_loadedNPCsUsePotions = true;
	}

	// animated poisons
	if (const uint32_t index = Mods::GetPluginIndex(AnimatedPoisons); index != 0x1) {
		_loadedAnimatedPoisons = true;
		loginfo("Found plugin AnimatedPoisons.esp.");
	}
	if (const uint32_t index = Mods::GetPluginIndex(AnimatedPoisons_5); index != 0x1) {
		_loadedAnimatedPoisons = true;
		loginfo("Found plugin Animated Poisons.esp.");
	}

	// animated potions
	if (const uint32_t index = Mods::GetPluginIndex(AnimatedPotions_4_4); index != 0x1){
		_loadedAnimatedPotions = true;
		loginfo("Found plugin {}.",AnimatedPotions_4_4);
	}
	if (const uint32_t index = Mods::GetPluginIndex(AnimatedPotions_4_3); index != 0x1){
		_loadedAnimatedPotions = true;
		loginfo("Found plugin {}.",AnimatedPotions_4_3);
	}

	// ZUPA
	if (const uint32_t index = Mods::GetPluginIndex(ZUPA); index != 0x1){
		loginfo("Found plugin {}.", ZUPA);
		_loadedZUPA = true;
	}

	// Sacrosanct
	if (const uint32_t index = Mods::GetPluginIndex(Sacrosanct); index != 0x1) {
	...
	// ordinator
	if (const uint32_t index = Mods::GetPluginIndex(Ordinator); index != 0x1) {
		loginfo("Found plugin Ordinator - Perks of Skyrim.esp.");
		_loadedOrdinator = true;
	}

	// vokrii
	if (const uint32_t index = Mods::GetPluginIndex(Vokrii); index != 0x1) {
		loginfo("Found plugin Vokrii - Minimalistic Perks of Skyrim.esp.");
		_loadedVokrii = true;
	}

	// adamant
	if (const uint32_t index = Mods::GetPluginIndex(Adamant); index != 0x1) {
